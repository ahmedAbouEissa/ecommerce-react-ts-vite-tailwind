...
const Product = () => {
  ...
  const checkSize = cartItems.find(
    (item: ProductsProps) => item.id == productData?.id && item.sizes === size
  );
      ...
      <div className="flex gap-12 sm:gap-12 flex-col sm:flex-row">
        ...
        <div className="flex-1">
          ...
          {size && checkSize ? (
            <>
              <button
                className="bg-black text-white px-2 py-1 mr-1 text-sm"
                onClick={() => updateCart(productData, size, "decrease")}
              >
                -
              </button>
              {checkSize.qty}
              <button
                className="bg-black text-white px-2 py-1 ml-1 text-sm"
                onClick={() => updateCart(productData, size, "increase")}
              >
                +
              </button>
            </>
